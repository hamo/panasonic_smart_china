...
项目当前基于对“松下智能家电”App 通信逻辑的分析实现，非松下官方项目。目前已支持品类代号 `0900` 的松下风管机/中央空调线控设备、品类代号 `0900` 的分体空调（`AirconCommon-2024-03`，如 `CS-J9AKT10` / `CS-J13AKT10`），以及品类代号 `0820` 的 `FV-RB20VL1` 风暖浴霸。代码内部采用 profile 驱动模型，后续会通过独立 profile、协议端点和实体能力映射扩展更多品类和设备型号。
...
3.0 版本在账号级配置模型基础上扩展了多品类 profile 支持，当前已注册 `0900` 风管机、`0900` 分体空调（AirconCommon）和 `0820` `FV-RB20VL1` 风暖浴霸三类 profile：

- 在 HA 中以松下账号为配置入口。
- 登录后自动扫描账号下可识别的设备。
- 每个设备会在同一账号配置项下创建对应实体。
- profile 可以声明品类代号、型号匹配、HA 平台、实体能力映射（switch/select/sensor/binary_sensor/button）、状态读取接口、控制接口、安全写入字段和命令级行为（如每次指令自动蜂鸣）。
...
- 引入 `DataUpdateCoordinator`：统一设备状态轮询与实体分发，支持乐观更新、会话失效检测与自动触发重新认证。其中 `PanasonicDeviceCoordinator` 负责主状态读取与控制下发，`PanasonicAuxCoordinator` 负责辅助端点（电量统计、每模式温度记忆）的低频轮询。
- TLS 指纹固定：对 `psmartcloud` 云 API 证书做指纹校验，降低中间人风险。
- Profile 驱动扩展层：设备能力全部通过 profile 声明（品类/型号匹配、协议端点、实体能力映射、安全写入字段），新增机型不再需要改动平台代码。
- Read-Modify-Write 控制：下发指令前先读取设备最新状态，只修改必要字段，避免覆盖设备真实状态。
...
- 0900 分体空调控制（`AirconCommon-2024-03`，如 CS-J9AKT10 / CS-J13AKT10）：
  - 开关机、自动/制冷/制热/抽湿/送风模式、目标温度、风速（自动/20-100%）。
  - 垂直/水平扫风、场景（关闭/怡静风/超远风/环抱风）。
  - 自清洁、柔湿制冷、强智冷、辅热、灯光、UVC 除菌、语音、空间净化（按设备能力显示）。
  - 新风（ERV）风机：低/中/高三档独立控制（运行状态与官方 App 一致：独立运行或空调开机带新风时视为开启，空调关机导致新风停止时显示关闭）。
  - 每次下发指令自动触发设备蜂鸣提示（`buzzer=65`，与官方 App 行为一致），无独立蜂鸣开关实体。
  - 回风温度、故障码（含已知故障说明）、故障、新风运行、滤网需更换状态。
  - 电量统计（60 秒轮询）：今日/本月用电量与运行时长和费用，以及本年用电量。
  - 每模式温度记忆：切换制冷/制热/抽湿时恢复该模式上次使用的温度（与官方 App 一致）。
  - 滤网重置按钮。
- FV-RB20VL1 风暖浴霸控制：支持待机、取暖、换气、凉干燥和热干燥模式。
- 静音风速映射（0900 风管机）：将松下协议中的静音开关映射为 HA 中的 `Quiet` 风速。
...
| `0900` | 分体空调 | `split_ac_0900_airconcommon` | `AirconCommon-2024-03`，如 `CS-J9AKT10` / `CS-J13AKT10` |
| `0820` | 风暖浴霸 | `bathroom_heater_0820_fv_rb20vl1` | 支持型号 `FV-RB20VL1`，设备 ID 后缀可能显示为 `Aircle-05-02` |

其他品类和型号暂未声明支持。即使能在扫描中识别出来，也需要补充 profile/实体能力映射并完成真实设备验证后再开放。
...
空调 profile（0900 风管机与 0900 分体空调）使用 1°C 温度步长，支持 16°C 到 30°C 的目标温度范围（协议内以 x2 标度传输）。
...
0900 分体空调（AirconCommon）风速档位为 `Auto` 与 `20%`/`40%`/`60%`/`80%`/`100%`，不提供静音档。
...
1. 稳定 0900 分体空调（AirconCommon）控制链路，持续收敛 read/write、会话失效、异常状态处理，并验证更多机型。
2. 基于当前 profile 模型继续补齐不同 HA 平台的实体能力映射，例如 climate、fan、switch、select、sensor、binary_sensor、button。
3. 吸收社区贡献的设备适配逻辑，逐步扩展为松下中国区 HA 集成的核心仓库。

仓库内的 `tools/probe_ac_endpoints.py` 提供了抓包式的协议探针工具，可用于验证新机型/新品类的端点和字段映射（含 `--watch` 实时观测 App 操作对应的字段变化）。

当前新增设备适配的推荐路径：

1. 新建设备 profile，声明 `profile_id`、`category_ids`、可选 `model_ids`、HA 平台、实体类型、状态读取接口、控制接口和响应校验字段。
2. 如果已有 HA 平台和实体类型可以复用，只补 profile；如果能力模型不同，再扩展对应的实体能力映射。
...
除非提交者明确标注 `Not a Contribution`，否则提交到本仓库的 Pull Request、issue、讨论、代码片段、设备 profile、实体能力映射和协议字段说明，均视为愿意按 Apache-2.0 授权给本项目使用。维护者可以基于这些内容提取、改写、合并设备适配逻辑，并发布到本仓库的后续版本中。
